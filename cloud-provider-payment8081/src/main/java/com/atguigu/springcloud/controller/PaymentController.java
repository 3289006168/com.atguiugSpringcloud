...
//    private String UrlName = "Payment";
    ...
    public CommonResult<Integer> CreatePayment(@RequestBody Payment payment){
        ...
        if (count>0){
            return new CommonResult<Integer>(200,"创建成功"+ServerPost,count);
        }
       else return new CommonResult<Integer>(404,"创建失败"+ServerPost);
    }
    ...
    public CommonResult<Payment> SelectPayment(@PathVariable("id") Integer id){
        ...
        if (paymentById!=null){
//            int a = 100;
            return new CommonResult<Payment>(200,"查询成功"+ServerPost,paymentById);
        }
        else return new CommonResult<Payment>(404,"查无此人"+ServerPost);
    }
}
